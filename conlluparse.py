...
class Word:
    def __init__(
        self, id, form, lemma, upostag, xpostag, feats, head, deprel, deps, misc
    ):
        self.id = id
        self.form = form
        self.lemma = lemma
    ...
    def compare(self, word, number, word_two):
        if number == 0:
            return (
                self.upostag.lower().strip() == word.lower().strip()
                or self.deprel.lower().strip() == word.lower().strip()
            )
        elif number == 1:
            return self.id == word_two.head and (
                compare_without_caps(self.upostag, word)
                or compare_without_caps(self.deprel, word)
                or compare_without_caps(self.form, word)
                or compare_without_caps(self.lemma, word)
            )
...
def inequality(user_input):
    ...
    for word in i.words:
        ...
        for no in input_one:
            if (
                compare_without_caps(input_one[no], word.text_to_attr(no.strip()))
                == False
            ):
                works_l = False
        if works_l:
            print(
                word.print_info()
                + sentences[overall_counter].id_name
                + sentences[overall_counter].print_sentence()
            )
...
for line in args.connlufile:
    ...
    if line[0] != "#" and line.strip() != "":  # check if it's not a comment
        ...
        line = re.sub(
            " +", ",", line
        )  # removes whitespaces to make it a comma for easy splitting
    ...
    if ">" in local_input:
        ...
        if not ("=" in local_input[0]) and not ("=" in local_input[1]):
            for word in i.words:
                for word_two in i.words:
                    if compare_without_caps(
                        local_input[0], word.form
                    ) and compare_without_caps(
                        local_input[1], word_two.form.strip().lower()
                    ):
            ...
            for word in i.words:
                ...
                for n in input_one:
                    if (
                        compare_without_caps(input_one[n], word.text_to_attr(n.strip()))
                        == False
                    ):
                        works = False
                if works:
            ...
            for word in i.words:
                ...
                for n in input_two:
                    if (
                        compare_without_caps(input_two[n], word.text_to_attr(n.strip()))
                        == False
                    ):
                        works = False
                if works:
        ...
        if not ("=" in local_input[1]) and not ("=" in local_input[0]):
            ...
            for word_two in i.words:
                for word in i.words:
                    if compare_without_caps(
                        local_input[1], word.form
                    ) and compare_without_caps(
                        local_input[0], word_two.form.strip().lower()
                    ):
                        if word.id == word_two.head:
                            if not word in wor:
                                wor.append(word)
